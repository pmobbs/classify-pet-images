...
def classify_images(images_dir, results_dic, model):
    ...
    for file_i in results_dic:
      ...
      # Check image name against classfier list
      classifier_list = image_classification.split(',')
      pet_image = clean_file_name(file_i).lower()
      image_class_match = 0
      for pet_type in classifier_list:
        clean_pet = (pet_type.strip()).lower()
        if clean_pet == pet_image:
          image_class_match = 1
      ...
      results_dic[file_i] = [pet_image]
      results_dic[file_i].append(image_classification) 
      results_dic[file_i].append(image_class_match)

    return None
  
# Replaces '_' with space, removes numbers and file type suffix
def clean_file_name(str):
  #print(str)
  newstr = str[:len(str)-4].replace('_', ' ')
  word_i = 0
  outstr = ""
